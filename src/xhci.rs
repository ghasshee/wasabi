...
impl PciXhciDriver {
    ...
    )> 
    {
        ...
        let rt_regs  = 
            unsafe { Mmio::from_raw(bar0.addr().add(cap_regs.as_ref().rtsoff())
    ...
    async fn run(xhc: Controller) -> Result<()> {
        info!("xhci: cap_regs.MaxSlots = {}", 
              xhc.cap_regs.as_ref().num_of_device_slots());
        info!("xhci:  op_regs.USBSTS   = {}",  
              xhc.op_regs.as_ref().usbsts());
        info!("xhci:  rt_regs.MFINDEX  = {}",  
              xhc.rt_regs.as_ref().mfindex());
        Ok(())
    }
}
...
// Address Array is utilized but the xHCI Scratchpad mechanism.
        ...
        for sb in table.iter_mut() {
            let buf = ALLOCATOR.alloc_with_options(
                Layout::from_size_align(page_size, page_size)
                    .map_err(|_| "could not allocate a scratchpad buffer")?,
            );
            let buf = unsafe {
